...
def evaluar(expr, contexto):
    ...
    # manejar bicondicional ↔
    if "↔" in expr:
        partes = expr.split("↔", 1)
        izq = partes[0].strip()
        der = partes[1].strip()
        return evaluar(izq, contexto) == evaluar(der, contexto)
    ...
    if "→" in expr:
        ...
        izq = partes[0].strip()
        der = partes[1].strip()
        return (not evaluar(izq, contexto)) or evaluar(der, contexto)

    # si no hay operadores especiales, traducir y evaluar directamente
